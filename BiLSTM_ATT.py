...
class BiLSTM_ATT(nn.Module):
    def __init__(self, config, embedding_pre):
        ...
        self.att_weight = nn.Parameter(
            torch.randn(self.batch, 1, self.hidden_dim))
        self.relation_bias = nn.Parameter(
            torch.randn(self.batch, self.tag_size, 1))
    ...
    def forward(self, sentence, pos1, pos2):
        ...
        embeds = torch.cat((self.word_embeds(sentence), self.pos1_embeds(pos1),
        ...
        relation = torch.tensor([i for i in range(self.tag_size)],
                                dtype=torch.long).repeat(self.batch, 1)
